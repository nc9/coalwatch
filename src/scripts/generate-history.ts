...
export async function generateHistoryData(): Promise<HistoryData> {
  try {
    ...
    debug(
      `Found ${unitCodes.size} unique units across ${facilityCodes.size} facilities`
    )
    ...
    debug(
      `Fetching data from ${formatApiDate(startDate)} to ${formatApiDate(endDate)}`
    )
    ...
    const historyByUnit: Record<
      string,
      Map<string, { totalPower: number; count: number; maxPower: number }>
    > = {}
    ...
    rows.forEach((row) => {
      ...
      const dayData = historyByUnit[unitCode].get(dayKey) || {
        totalPower: 0,
        count: 0,
        maxPower: 0,
      }
